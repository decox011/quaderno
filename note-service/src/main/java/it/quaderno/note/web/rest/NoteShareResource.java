...
import it.quaderno.note.web.rest.errors.ElasticsearchExceptionMapper;
...
public class NoteShareResource {
    ...
    /**
     * {@code SEARCH  /note-shares/_search?query=:query} : search for the noteShare corresponding
     * to the query.
     *
     * @param query the query of the noteShare search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<NoteShareDTO>> searchNoteShares(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of NoteShares for query {}", query);
        try {
            Page<NoteShareDTO> page = noteShareService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
